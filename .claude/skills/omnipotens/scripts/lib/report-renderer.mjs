...
function directionBlocks(summary) {
  return [...Object.values(summary.executiveSummary), ...summary.additionalAnalyses]
    .map((item) => `<article class="direction"><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.summary)}</p>${missingBlock(item)}</article>`)
    .join('');
}
...
function renderAnalysisSummary(summary) {
  const audience = summary.executiveAudience;
  ...
  const overallBlock = `<article class="overall-assessment"><h3>総合評価：${escapeHtml(overall.label)}</h3><p class="overall-score">${overall.score} / ${overall.maxScore}</p><p>${escapeHtml(overall.summary)}</p><h4>主な強み</h4>${list(overall.strengths)}<h4>優先して解く課題</h4>${list(overall.priorityIssues)}<p><b>評価の確度:</b> ${escapeHtml(overall.confidence)}</p><p><b>参照:</b> ${overall.sourceRefs.map(escapeHtml).join(' / ')}</p>${missingBlock(overall)}</article>`;
  const executive = `<section class="executive-summary"><h2>エグゼクティブサマリ</h2><p class="audience-note"><b>一般読者・高校生向け。</b> 読解力は偏差値${audience.assumedAcademicDeviation}程度を想定しています。${escapeHtml(audience.audience)} ${audience.writingPolicy.map(escapeHtml).join(' / ')}</p>${overallBlock}<h3>各項目のまとめ</h3>${directionBlocks(summary)}</section>`;
