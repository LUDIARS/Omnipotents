...
function renderAnalysisSummary(summary) {
  ...
  return `<section class="profiles"><h2>エグゼクティブサマリ</h2><div class="profile-switch" role="tablist" aria-label="表示の詳しさ"><button type="button" data-profile-button="beginner" aria-selected="true">学生・初学者向け</button><button type="button" data-profile-button="highResolution" aria-selected="false">高解像度データ</button></div><div class="profile-panel" data-profile="beginner">${directionBlocks(summary, 'beginner')}</div><div class="profile-panel" data-profile="highResolution" hidden>${directionBlocks(summary, 'highResolution')}</div></section>${scoreTable('AI Format スコア', summary.aiFormatScores)}${scoreTable('Vitia 市場性スコア（高い順）', summary.vitiaScores, true)}<section><h2>Ludusの新規性評価</h2><p><b>${novelty.score} / ${novelty.maxScore}</b> — ${escapeHtml(novelty.rationale)}</p>${missingBlock(novelty)}<h2>遊びの辞書からUXへつなぐ実装提案</h2>${recommendations}</section>`;
}
